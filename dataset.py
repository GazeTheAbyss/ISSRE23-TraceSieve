...
def trace_graph_to_dgl(graph: TraceGraph,
    ...
    with T.no_grad():
        ...
        # g.ndata['node_type'] = T.one_hot(
        #     T.as_tensor(
        #         np.minimum(gv.node_type, num_node_types - 1),
        #         dtype=T.int64
        #     ),
        #     num_node_types,
        #     dtype=T.float32,
        ...
        # # flag to indicate whether or not a node is root node
        # is_root = np.zeros([graph.node_count], dtype=np.float32)
        # is_root[0] = 1.
        # g.ndata['is_root'] = T.as_tensor(is_root)
        ...
        if USE_MULTI_DIM_LATENCY_CODEC:
            ...
            for pfx in ('avg_', 'max_', 'min_'):
                ...
                for i in range(graph.node_count):
                    ...
                    latency.append((latency_array[i] - mu) / std)
...
class TraceGraphDataStream(mltk.data.MapperDataStream):
    ...
    def __init__(self,
        ...
        source = mltk.DataStream.int_seq(
            len(db),
